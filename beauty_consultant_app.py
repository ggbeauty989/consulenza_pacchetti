...
with st.sidebar:
    ...
    # API Key — prima cerca nei Secrets di Streamlit Cloud, poi nel campo manuale
    ...
    if api_key_resolved:
        # Chiave trovata nei Secrets — connessione automatica, nessun campo visibile
        try:
            st.session_state.client = Anthropic(api_key=api_key_resolved)
            st.session_state.api_key_ok = True
            st.success("✅ Connessa a Claude!")
        except Exception:
            st.error("❌ API key nei Secrets non valida")
            st.session_state.api_key_ok = False
    else:
        # Secrets non configurati — mostra il campo manuale (utile in locale)
        st.markdown("### 🔑 API Key Claude")
        api_key_input = st.text_input(
            "Inserisci la tua API key Anthropic",
            type="password",
            placeholder="sk-ant-...",
            help="Oppure configurala nei Secrets di Streamlit Cloud",
        )
        if api_key_input:
            try:
